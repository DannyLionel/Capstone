...
$connect = mysqli_connect("localhost", "root", "root", "sample");

function picture_query($connect)
{
 $profile = $_GET['profileid'];
 $query = "SELECT * FROM profile_data WHERE type='picture' AND profile_name='$profile' ORDER BY id ASC";
...
function make_slides($connect)
{
 ...
 while($row = mysqli_fetch_array($result))
 {
  ...
  $output .= '
   <img src="data/'.$row["media"].'" alt="'.$row["id"].'" />
